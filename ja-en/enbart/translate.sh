...
for prefix in valid test test1 test2 test3; do
    ...
    sp_file=$TRANSLATION/${MODEL:${#DIR}}.$prefix.sp.en
    tok_file=$TRANSLATION/${MODEL:${#DIR}}.$prefix.tok.en
    detok_file=$TRANSLATION/${MODEL:${#DIR}}.$prefix.detok.en

    if [ ! -f ${sp_file} ]; then
        CUDA_VISIBLE_DEVICES=$GPU fairseq-generate $BIN_DATA \
            --path $point \
            --scoring sacrebleu \
            --gen-subset $prefix -s ja -t en > $sp_file
    fi
    ...
    # id-to-words and de-tokenize
    if [ ! -f $detok_file ]; then
       python $tools/decode.py $ENBART/encoder.json $tok_file > $detok_file
    fi
    ...
    cat $detok_file | sacrebleu -w 2 $DATA/$prefix.tok.en
    cat $detok_file | sacrebleu -w 2 $DATA/$prefix.tok.en >> $RESULTS
